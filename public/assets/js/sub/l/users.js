...
const fetchUsers = (start = null, limit = null, empty = true, cb = null, url = '/l/users/fetch') => {
...
const addToUsersList = (data) => {
    ...
    const {id,first_name,last_name,email,createdAt,status,rank} = data;
...
//         const el = e.currentTarget;
//         const ISBN = el.attr('data-del');
//         // impression
//         SET.removeElem(el.getParent('r-item'));
//         SET.ajax({
//             method: 'DELETE',
//             url: `/l/users/delete/${ISBN}`,
//             headers: {
//                 'Content-Type': false
//             },
//             handler: (res, err) => {
...
//     }
// }
// const activateUsersActions = () => {
//     let statusBtns = usersTableBody.getElem('button[data-status]', true);
//     let deleteBtns = usersTableBody.getElem('button[data-del]', true);
 ...
 * refresh
 */
const refreshBtn = SET.$('#refreshUsers');
refreshBtn.on('click', () => {
    SET.fixClass([refreshBtn],[['spin']],[true]);
    refreshBtn.attr('disabled', true);
    fetchUsers(null,null,true,() => {
...
SET.$('form#searchUsers').on('submit', e => {
    ...
    if (searchValue.trim().length > 0) {
        fetchUsers(null,null,true,null,`/l/users/search?q=${searchValue}`);
    }
});